...
<template>
  <header class="w-full bg-black h-20 relative z-50">
    <div class="xl:flex w-full h-full justify-center items-center">
      ...
      <div>
        <Bars4Icon
          class="h-10 mt-5 cursor-pointer absolute xl:hidden text-white"
          @click="modalBar = !modalBar"
        />
      </div>
      <Sucess
      ...
      >
        ...
        <div class="relative flex items-center gap-1">
          ...
          <div
            ref="target"
            v-if="modal2"
            class="absolute top-12 gap-3 text-slate-300 flex flex-col z-50 bg-black py-2 px-4"
            @mouseleave="modal2 = false"
          >
            <RouterLink
              :to="{ path: '/clothes', query: { gender: 'male' } }"
              @click="modal2 = false"
              >Masculino
            </RouterLink>
            <RouterLink
              :to="{ path: '/clothes', query: { gender: 'female' } }"
              @click="modal2 = false"
              >Feminino
            </RouterLink>
          </div>
        ...
        <RouterLink class="flex items-center gap-1" to="/"
          ><PaperClipIcon class="h-4" /> Sobre nós</RouterLink
        >
        <RouterLink
          to="/administrador"
          class="text-red-800 hover:text-red-500 font-bold"
          v-if="acessLevel"
          >Administrador
        </RouterLink>
        <div class="right-28 absolute gap-10 flex items-center">
          <div class="relative">
            <ShoppingCartIcon
              @click="store.modalCart = !store.modalCart"
              class="h-6 cursor-pointer text-orange-600 z-50 hover:text-orange-200"
            />
            <span
              class="absolute bottom-4 right-0 text-xs text-slate-300"
              v-if="store.countCart"
              >{{ store.countCart }}</span
            >
          </div>
          <RouterLink
            to="/auth/acesso/"
            class="right-14 absolute flex items-center"
            v-if="!logged"
          >
            <span class="text-md w-36 flex items-center gap-1">
              <UserIcon class="h-4 mr-1 text-sm text-orange-600" /> Entrar</span
            >
          </RouterLink>
      ...
      <Transition>
        ...
        >
          <div class="h-20 w-full flex justify-between items-center bg-black">
            <h1 class="font-semibold text-gray-100 text-center border-b-2 ml-4">
              Shopping <span class="text-orange-700">Oriaj</span>
            </h1>
            <XCircleIcon
              class="text-white h-8 mr-4 cursor-pointer"
              @click="modalBar = false"
            />
          </div>
          <div class="gap-10 flex flex-col p-2 m-2">
            <div>
              <h1 v-if="name">
                Ola, <span class="text-orange-700 text-sm">{{ name }}</span>
              </h1>
            </div>
            ...
            <div class="relative w-full cursor-pointer">
              <div @click="modal1 = !modal1">
                <h1 class="cursor-pointer flex gap-1">
                  Vestuário
                  <ChevronUpIcon
                    v-if="!modal1"
                    class="h-4 absolute top-2 right-4 qq"
                  />
            ...
            <RouterLink
              to="/auth/acesso/"
              class="right-28 xl:absolute flex items-center"
              @click="closeModal"
              v-if="!logged"
            >
              Entrar
            ...
            <RouterLink
              to="/"
              class="right-28 xl:absolute flex items-center gap-4"
              @click="logout"
              v-else
            >
              Sair
      ...
      <div class="absolute right-5 top-8 xl:hidden md:block">
        ...
        />
        <span
          class="absolute bottom-4 right-0 text-xs text-gray-600"
          v-if="store.countCart"
          >{{ store.countCart }}</span
        >
      </div>
