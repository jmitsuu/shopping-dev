...
<template>
    <main class="relative flex justify-center items-center">
        <Transition>
        <div v-if="scroll" class="fixed z-50  bottom-8 h-20 w-[600px] justify-center gap-10 p-4 items-center flex rounded-md border-2 bg-[#F8F8FF] shadow-xl" >
       <h2 class="left-8 absolute"> {{ name }}</h2>
            <UserIcon class="h-6 cursor-pointer text-orange-600 hover:text-orange-200"  />
     <ShoppingCartIcon @click="store.modalCart = !store.modalCart" class="h-6 cursor-pointer text-orange-600 hover:text-orange-200" />
        </div>
    </Transition>
    </main>
</template>
