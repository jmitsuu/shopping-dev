...
<template>
  <main class="relative">
    ...
    >
      ...
      />
      <h1 class="text-3xl font-bold uppercase text-gray-500 border-b-2 mb-10">
        Carrinho
      </h1>
      <div class="max-h-[600px] overflow-auto p-2">
        <div
          class="w-full flex gap-5 mb-3 items-center border-2 p-1 rounded-md"
        ...
        >
          ...
          <h2 class="text-xs">
            {{ item.title }} -
            <span class="text-orange-600">{{ item.price }} R$</span>
          </h2>
          <h3
            class="text-red-500 text-xs cursor-pointer hover:text-red-600"
            @click="store.removeItemCart(index)"
          >
            remover
          </h3>
        </div>
      ...
      <div>
        <h2 class="text-xl uppercase mp-2 border-b-2">
        Total:<span class="text-orange-700 font-bold text-2xl pl-1">{{
          store.total
        }}</span>
      </h2>
        <button
